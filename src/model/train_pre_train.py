...
def train_test_epochs(generator, discriminator, train_loader, test_loader, epochs, lr, device, base_save_path, pre_train_model=False, lamb=100):
    ...
    if start_epoch == 0:
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc="Initial Testing"):
                ...
                loss_D_fake = nn.BCEWithLogitsLoss()(fake_prediction, torch.zeros_like(fake_prediction))
                real_images = torch.cat([gray_images, ab_images], dim=1)
                real_prediction = discriminator(real_images)
                loss_D_real = nn.BCEWithLogitsLoss()(real_prediction, torch.ones_like(real_prediction))
                loss_D = (loss_D_fake + loss_D_real) / 2
                
                fake_images = torch.cat([gray_images, fake_ab], dim=1)
                fake_preds = discriminator(fake_images)
                generator_bce = nn.BCEWithLogitsLoss()(fake_preds, torch.ones_like(fake_preds))
                l1_loss = F.l1_loss(fake_ab, ab_images)
...
if __name__ == "__main__":
    ...
    generator = generator.to(device)
    pretrained_path = "/accounts/grad/phudish_p/CS280A_final_project/pretrained_resent_test.pt"
    ...
    print(f"base_save_path: {base_save_path}")
    train_losses, test_losses = train_test_epochs(generator, discriminator, train_dataloader, test_dataloader, epochs, lr, device, base_save_path, pre_train_model=False)

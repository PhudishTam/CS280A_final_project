...
def save_checkpoint(generator, discriminator, optimizer_G, optimizer_D, epoch,
                    ...
                    train_discriminator_losses, test_discriminator_losses, train_generator_vq_losses, test_generator_vq_losses,
                    base_save_path):
    if (epoch + 1) % 2 == 1:
    ...
    checkpoint_G = {
        ...
        'train_generator_vq_losses': train_generator_vq_losses,
        'test_generator_vq_losses': test_generator_vq_losses
    }
...
def load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, base_save_path):
    ...
    if os.path.exists(save_path_G) and os.path.exists(save_path_D):
        ...
        train_generator_vq_losses = checkpoint_G.get('train_generator_vq_losses', [])
        test_generator_vq_losses = checkpoint_G.get('test_generator_vq_losses', [])
        ...
        return epoch, train_generator_losses, test_generator_losses, train_discriminator_losses, test_discriminator_losses, train_generator_bce_losses, test_generator_bce_losses, train_generator_l1_losses, test_generator_l1_losses, train_generator_vq_losses, test_generator_vq_losses
    else:
...
def train_test_epochs(generator, discriminator, train_loader, test_loader, epochs, lr, device, base_save_path, lamb=100):
    ...
    start_epoch, train_generator_losses, test_generator_losses, train_discriminator_losses, test_discriminator_losses, train_generator_bce_losses, test_generator_bce_losses, train_generator_l1_losses, test_generator_l1_losses, train_vq_losses, test_vq_losses = load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, base_save_path)
    if start_epoch == 0:
        generator.eval()
        discriminator.eval()
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc="Initial Testing"):
                ...
                gray_images_pretrain = gray_images.clone().repeat(1, 3, 1, 1)                
                fake_ab,vq_loss = generator(gray_images_pretrain)
                ...
                loss_G = generator_bce + lamb * l1_loss + vq_loss 
                ...
                initial_vq_loss += vq_loss.item()
        initial_generator_test_loss /= len(test_loader)
        initial_discriminator_test_loss /= len(test_loader)
        initial_generator_bce_loss /= len(test_loader)
        initial_generator_l1_loss /= len(test_loader)
        initial_vq_loss /= len(test_loader)
        ...
        test_vq_losses.append(initial_vq_loss)
        ...
        print(f"Initial VQ Test Loss: {initial_vq_loss}")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        test_vq_loss /= len(test_loader)
        test_vq_losses.append(test_vq_loss)
        ...
        print(f"Epoch {epoch+1}/{epochs} Generator Train VQ Loss: {np.mean(train_vq_losses)}")
        ...
        print(f"Epoch {epoch+1}/{epochs} Generator Test VQ Loss: {test_vq_loss}")
        ...
        save_checkpoint(generator, discriminator, optimizer_G, optimizer_D, epoch, train_generator_losses, train_generator_bce_losses,train_generator_l1_losses,
                        test_generator_losses, test_generator_bce_losses,test_generator_l1_losses ,train_discriminator_losses, test_discriminator_losses, train_vq_losses, test_vq_losses, base_save_path)
...
if __name__ == "__main__":
    ...
    test_dataset = Datasetcoloritzation(test_data_dir, device=device, training=False, image_size=256)
    ...
    generator = VQResNetUNet(num_embeddings=2048,commitment_cost=0.25,out_channels_decoder=512)
    pretrained_path ="/accounts/grad/phudish_p/CS280A_final_project/pretrained_vq_GAN_test.pt"
    if os.path.exists(pretrained_path):
        print(f"Loading pretrained generator from {pretrained_path}")
        generator.load_state_dict(torch.load(pretrained_path))
    else:
        print(f"No pretrained generator found at {pretrained_path}")
    ...
    print(f"Saving checkpoints to {base_save_path}")
